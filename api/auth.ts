...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (method === 'POST') {
    if (url?.includes('/login')) {
      ...
      try {
        const { data: user, error } = await supabase.from('utilisateurs').select('*').eq('email', email).maybeSingle();
      ...
      try {
        const { data: existingUser } = await supabase.from('utilisateurs').select('id').eq('email', email).maybeSingle();
        if (existingUser) return res.status(400).json({ error: 'Cet email est déjà utilisé.' });

        const hashedPassword = await bcrypt.hash(mot_de_passe, 10);
        const { data: user, error } = await supabase.from('utilisateurs').insert([{
          email, mot_de_passe: hashedPassword, nom, prenom, telephone, ville, adresse, role: 'client'
        }]).select().single();

        if (error) throw error;

        const token = jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || "fallback_secret", { expiresIn: '7d' });
        return res.status(201).json({ token, user: { id: user.id, email: user.email, nom: user.nom, prenom: user.prenom, role: user.role } });
      } catch (err) {
        console.error(err);
    ...
    if (url?.includes('/me')) {
      ...
      try {
        const { data: userData, error } = await supabase.from('utilisateurs').select('id, email, nom, prenom, telephone, ville, adresse, role').eq('id', (user as any).id).single();
